...
<div class="card-body">
	<form action="{{url('/admin/servicios/'.$servicio->id)}}" method="post" enctype="multipart/form-data">
        ...
        <div class="form-group row {{ $errors->has('nombre') ? ' has-warning' : '' }}">
            <label class="col-sm-4 col-form-label" for="estado">
                Estado del servicio
            </label>
            <div class="form-check-inline my-1">
                <label>
                <input type="radio" name="estado" id="estado" value="0" {{$servicio->estado == 0 ? 'checked="checked"' : ''}}>
                <i class="helper"></i>En proceso
                </label>
            </div>
            <div class="form-check-inline my-1">
                <label>
                <input type="radio" name="estado" id="estado" value="1" {{$servicio->estado == 1 ? 'checked="checked"' : ''}}>
		...
		<div class="form-group row {{ $errors->has('nombre') ? ' has-warning' : '' }}">
            ...
            <div class="col-sm-8">
            	<input class="form-control {{ $errors->has('nombre') ? ' form-control-warning' : '' }}" type="text" id="nombre" name="nombre" value="{{$servicio->nombre}}" >
        ...
        <div class="form-group row {{ $errors->has('descripcion') ? ' has-warning' : '' }}">
            ...
            <div class="col-sm-8">
                <textarea  rows="5" class="form-control {{ $errors->has('descripcion') ? ' form-control-danger' : '' }}" id="descripcion" name="descripcion">{{ $servicio->descripcion }}</textarea>
        ...
        <div class="form-group row {{ $errors->has('descripcion2') ? ' has-warning' : '' }}">
            ...
            <div class="col-sm-8">
                <textarea  rows="5" class="form-control {{ $errors->has('descripcion2') ? ' form-control-danger' : '' }}" id="descripcion2" name="descripcion2">{{ $servicio->descripcion2 }}</textarea>
        ...
        <div class="form-group row {{ $errors->has('precio') ? ' has-warning' : '' }}">
            ...
            <div class="col-sm-8">
            	<input class="form-control {{ $errors->has('precio') ? ' form-control-warning' : '' }}" type="number" id="precio" name="precio" value="{{$servicio->precio}}">
        ...
        <div class="form-group row {{ $errors->has('materiales') ? ' has-warning' : '' }}">
            ...
            <div class="col-sm-6">
                ...
                @foreach( $servicio->materiales as $material)
                    <span class="badge badge-success">{{$material->nombre}}</span>
                @endforeach
            ...
            <input class="form-control" type="hidden" id="materiales_selected" name="materiales_selected">
        </div>
